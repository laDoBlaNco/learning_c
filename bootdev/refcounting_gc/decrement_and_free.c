/**Decrement and Free:
 *
 * Now things are going to get a touch more complicated and thus
 * interesting. In a refcounting GC, the interesting stuff happens
 * when the refcount equals 0. That's when the garbage gets collected
 *
 * Whent he refcount reaches 0, there are no references to this object
 * anymore! So no need to keep it around, meaning we need to free it
 * automagically which is the whole point of a garbage collector.
 *
 * For my first pass at this, I'll only handle integers, floats, and
 * strings to keep things simple. As I go a little further, I'll
 * implement it for th erest of the SnekObject types.
 */
...
int main(int argc, char const *argv[])
{
    ...
    printf("\nTest - test_inc_refcount, test_dec_refcount, test_free_refcount\n");
    snek_object_t *obj2 = new_snek_float(4.20);
    printf("\tRefcount should be 1 on creation and it is: %d\n", obj2->refcount);
    refcount_inc(obj2);
    printf("\tRefcount should be incremented to 2 and it is: %d\n", obj2->refcount);
    refcount_dec(obj2);
    printf("\tRefcount should be decremented to 1 and it is: %d\n", obj2->refcount);
