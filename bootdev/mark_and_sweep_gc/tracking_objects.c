/**Tracking Objects:
 *
 * My virtual machine needs to track EVERY snek object that gets
 * created.
 *
 * I'm no longer going to track how many times an object is referenced
 * but instead check at garbage collection time if each object is still
 * referenced at all. If it is, keep it, if not free it
 *
 */
#include <stdio.h>
#include <stdbool.h>
