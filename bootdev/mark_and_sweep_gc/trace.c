/**Trace:
 *
 * Now that we've done th first (and easiest) part: marking, we can
 * trace through all of our objects and determine which ones are
 ...
 *          return [a]
 *      print(get_list())
 * If we run this code it'll return a list with the integer a inside
 * it. Our current mark function will mark the list, but it won't
 * mark the integer 'a'. Which means tha when we go to sweep the
 * memory, we will mark the list, but not the integer a. We'd then
 * free a while its still being used, and the OS could then fill that
 * memory with something else... which would be very bad
 *
 * Also:
 *      def get_list():
 *          a = []
 ...
 *      print(get_list())
 *
 * in the above we create a list that references itself and then
 * returns a completely unrelated list. If our trace function looks
 * for any object that is referenced by any other object it will
 * consider a alive because it ahs a reference (albeit, to itself
 * in this case). In fact, a is unreachable because when get_list
 * returns, a is no longer used anywhere.
 *
 * Tracing solves both of these problems. To be clear, tracing is
 * part of the 'mark' phase of mark and sweep. It's where we mark
 * all the objects referenced by our root objects. So Mark Phase is
 * really a 2 part process. First root objects and then everything
 * else referenced.
 */
#include <stdio.h>
#include <stdbool.h>
