/**Free:
 *
 * Remember the refcount_free function that I wrote in the other 
 * chapter? It used a switch and depending on the type (obj->kind)
 * it would take the proper steps to free the associated memory
 * 
 * I need to re-write it now for a MaS GC. Since the vm is the one 
 * tracking objects, all of the refcount_dec work can be removed
 * There's some very cool tricks coming up for MaS to manage this, 
 * but for now I'll trust that I'll be correclty freeing any of the
 * constrained elements if they are no longer alive.
 * 
 * 
 */
#include <stdio.h>
...
void snek_objet_free(snek_object_t *obj)
{
    switch(obj->kind)
    {
        case INTEGER:
